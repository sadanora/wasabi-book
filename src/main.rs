...
const _: () = assert!(offset_of!(EfiSystemTable, boot_service) == 96);
...
fn locate_graphic_protocol<'a>(
    ...
    if status != EfiStatus::Success {
        return Err("Failed to locate grapics output protocol");
    }
...
fn draw_point<T: Bitmap> (
    ...
    *(buf.pixel_at_mut(x, y).ok_or("Output of Range")?) = color;
    Ok(())
}
